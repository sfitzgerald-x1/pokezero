...
def _metadata(*, corpus_sha256: str, shards: Mapping[str, str], pair_sha256: str,
              ...
              source_reproduction: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema": oi.SCHEMA,
...
def write_cells_new(out_dir: Path, cells: Mapping[str, list[dict[str, Any]]], *,
                    ...
                    source_reproduction: Mapping[str, Any]) -> None:
...
def _parse(spec: str) -> tuple[str, Path]:
    ...
    if not name or marker != "=" or not value or name == SOURCE_NAME:
...
def run(args: argparse.Namespace) -> None:
    ...
    def make_env() -> Any:
        ...
        if source_belief is not None and getattr(env, "belief_set_source_hash", None) != source_belief:
            _refuse("source checkpoint belief-set provenance differs from replay environment")
        if source_belief is None and not args.allow_unstamped_belief:
            _refuse("source checkpoint lacks belief-set provenance; pass --allow-unstamped-belief explicitly")
        return env
    ...
    source_config = RolloutConfig(max_decision_rounds=args.source_max_decision_rounds)
    for ordinal, seed in enumerate(sorted(by_seed), start=1):
        ...
        for want in wanted:
            ...
            if source_delta > args.reproduce_tol:
                _refuse(f"source head reproduction exceeds {args.reproduce_tol:g} at "
...
def main() -> int:
    ...
    if args.source_max_decision_rounds != 250 or args.reproduce_tol < 0:
        print("REFUSING: source cap must remain 250 and reproduction tolerance nonnegative", file=sys.stderr)
        return 2
    try:
        run(args)
