...
The config file is versioned JSON and records the suggested audit thresholds plus source and calibration metadata. `audit`, `compare --audit-config`, and post-iteration self-play flags can consume it. Explicit threshold flags still override values from the config file.
