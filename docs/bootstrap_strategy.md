...
- Start self-play from the bootstrap checkpoint and compare against cold-start runs using the self-play report command. A first matched smoke-profile comparison exists; the next useful comparison should use stricter pilot-derived or named long-run guardrails.
