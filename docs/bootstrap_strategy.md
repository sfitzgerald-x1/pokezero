...
Add `--audit-profile smoke`, `--audit-profile default`, or `--audit-profile long-run` to include per-run audit pass/fail status and failed audit checks in the same comparison output. The compare command remains read-only and does not run new games; audit failures are shown as row health signals rather than command failures unless a manifest itself cannot be loaded.
