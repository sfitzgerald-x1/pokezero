...
| historical self checkpoints | frozen nets | matched | v2+ current-family only; enforce with `opponents.py` `current_family_*` helpers; curated milestones in the configured artifact store + `checkpoints/curated/` |
