...
| Deadline-safe candidate publication | Fresh create-only R9 built the current source `853d7e544d861b2dc356195c8dd24581a4406bc5` through its detached-checkout, engine-vendoring, Cargo-vendoring, and image-build records, then wrote terminal `NONPASS` at `push_source_image`. The registry returned HTTP 403 `unauthorized` for the new tag. The Eigen-managed `crusoe-registry` credential metadata was unchanged, so no digest-qualified image or deadline source receipt exists for this source. | The current source can build on the selected CPU node; the remaining source-admission blocker is registry publication. | A deadline qualification, same-deadline behavior, an image identity, or strength. Do not retry the identical build until the controller-managed credential has push scope. |
