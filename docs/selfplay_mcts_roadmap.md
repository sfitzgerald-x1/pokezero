...
- **Self-play iterate loop** (`neural_cli iterate` → `neural_selfplay.py`, `selfplay.py`,
  ...
  train-call elapsed time, checkpoint size, final metrics, and cache lifecycle bytes; use that artifact
  rather than external logs when judging the 20k-games-under-10-minutes training gate.
