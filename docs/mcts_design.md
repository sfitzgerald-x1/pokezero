...
> replay-materialization roadmap. Its W5 redesign is implemented, with fresh bounded mechanics and
> telemetry validation as the next gates on an image containing the direct-world fixes. The prior
> mechanics attempt failed closed before it exercised direct materialization. The pre-W5 diagnostics
> retained below are historical baseline evidence, not a description of the active materialization
> mechanism.
...
| **Belief determinizer / start overrides** | **built; W5 validation in progress** | `belief.py` emits concrete opponent realizations from the belief view, and the Gen 3 planner materializes our request-known team plus sampled public-belief opponent teams. Tier 2 now constructs the current branch point from that sampled world plus the public source state, rather than requiring a full sampled-team root replay to reproduce history. It remains PIMC-style averaging rather than an information-set tree, and unsupported public effects fail closed. The W5 validation sequence measures whether this removes the historical replay-driven fallback problem without reducing FoulPlay strength. |
...
- **Forking (WS-C) + compute budget (the real viability gate):** W5 replaces current-state
  ...
  branch/visit. The W5 validation sequence measures the post-redesign per-move cost and fallback rate before
