...
| **Belief determinizer / start overrides** | **partial, smoke-brittle** | `belief.py` emits concrete opponent realizations from the belief view, and replay/search can now accept a `BattleStartOverride` or callable start-override source. Overrides are deliberately restricted to complete `p1`/`p2` packed teams in `gen3customgame`, because arbitrary teams are not honored by `gen3randombattle`; replay checks the searched player's pre-action observation features before recorded prefix actions are submitted, excluding instance metadata and opponent-private POV. The Gen 3 randbat start-override planner can now materialize our request-known team plus sampled public-belief opponent teams, preserve observed move-slot ordering, filter public HP-compatible variants, use Gen 3 source metadata, and retry sampled worlds per opponent-action scenario. Hidden-mode foul-play smoke still rejects most later-game sampled worlds during replay-prefix consistency checks, so this is not yet a viable strength path. |
...
4. **Current-state materialization for hidden-info ladder play.** The searcher has the first
   ...
   opponent variants, fills hidden backline candidates, preserves observed move-slot ordering, and
...
and one leaf-rollout round on seed `910001`, root-PUCT lost `0/1`, searched only 7 of 32 decisions,
fell back on 25 decisions, used 13 accepted start overrides, and spent 386 start-override attempts.
