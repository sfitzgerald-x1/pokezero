...
| **Belief determinizer / start overrides** | **partial, smoke-brittle** | `belief.py` emits concrete opponent realizations from the belief view, and replay/search can now accept a `BattleStartOverride` or callable start-override source. Overrides are deliberately restricted to complete `p1`/`p2` packed teams in `gen3customgame`, because arbitrary teams are not honored by `gen3randombattle`; replay checks the searched player's pre-action observation features before recorded prefix actions are submitted, excluding instance metadata and opponent-private POV. The Gen 3 randbat start-override planner can now materialize our request-known team plus sampled public-belief opponent teams, filter only request-known absolute HP-compatible variants, use Gen 3 source metadata, and retry sampled worlds per opponent-action scenario. It deliberately does not use opponent-private trajectory observations to reorder or filter opponent moves. Hidden-mode foul-play smoke still rejects most later-game sampled worlds during replay-prefix consistency checks, so this is not yet a viable strength path. |
...
4. **Current-state materialization for hidden-info ladder play.** The searcher has the first
   ...
   opponent variants, fills hidden backline candidates, applies only public/request-known filters, and
   can retry multiple sampled worlds per opponent-action scenario. The remaining missing piece is a
   way to branch from the **current public battle state** without relying on a sampled full-team root
   replay to reproduce every prior damage roll, volatile, item interaction, and status transition.
...
and one leaf-rollout round on seed `910001`, root-PUCT lost `0/1`, searched only 3 of 35 decisions,
fell back on 32 decisions, used 6 accepted start overrides, and spent 480 start-override attempts.
The dominant fallback remains replay-prefix observation mismatch at earlier decision rounds. That
points away from more small-sample MCTS tuning and toward current-state materialization or a
reversible simulator backend before any >=300-game MCTS strength read is meaningful.
