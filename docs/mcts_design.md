...
> replay-materialization roadmap. Its W5 redesign is implemented, while fresh mechanics and paired
> FoulPlay validation remain pending on an image containing the direct-world fixes. The prior
> mechanics attempt failed closed before it exercised direct materialization; no W5 validation Job
> is active. The pre-W5 diagnostics retained below are historical baseline evidence, not a
> description of the active materialization mechanism.
...
| **Belief determinizer / start overrides** | **built; W5 validation pending** | `belief.py` emits concrete opponent realizations from the belief view, and the Gen 3 planner materializes our request-known team plus sampled public-belief opponent teams. Tier 2 now constructs the current branch point from that sampled world plus the public source state, rather than requiring a full sampled-team root replay to reproduce history. It remains PIMC-style averaging rather than an information-set tree, and unsupported public effects fail closed. The next W5 jobs will measure whether this removes the historical replay-driven fallback problem without reducing FoulPlay strength. |
...
multi-belief path is wired, not that it is strong enough for a headline read. The foul-play harness
now defaults to `--start-override-attempts 10`, matching the cited randbat determinization recipe;
passing a lower value is a fast-smoke override, not the recommended hidden-world validation default.
...
- **Forking (WS-C) + compute budget (the real viability gate):** W5 replaces current-state
  ...
  branch/visit. The W5 validation jobs will measure the post-redesign per-move cost and fallback rate before
