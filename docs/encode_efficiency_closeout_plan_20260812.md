...
| Stage 1, first bite (#1219) | **MERGED (completed)** | all 8 per-leaf `format!` md keys retired via `md_key!`, which derives both spellings from one suffix token so a transposition is inexpressible; `get(md, &format!(…))` is gone from `encoder.rs` |
