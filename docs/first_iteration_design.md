...
- Configurable promotion gate CLI over bootstrap and self-play manifests using per-opponent benchmark win rates, minimum game counts, capped-game rates, and teacher-degradation counters.
...
- Evaluation exists against fixed baselines and historical checkpoints, with configurable absolute-floor promotion gates; incumbent-delta rules and long-run experiment criteria are still informal.
...
- Formal incumbent-delta promotion, long-run benchmark thresholds, regression detection, or checkpoint pool curation.
