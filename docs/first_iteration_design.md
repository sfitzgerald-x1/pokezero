...
- Source provenance snapshots in teacher bootstrap, linear self-play, neural self-play, CPU smoke, CPU pilot-suite, and CPU long-run wrapper artifacts, including repo root, branch, head, and dirty marker when git metadata is available.
...
- Evaluation exists against fixed baselines and promoted historical checkpoints, with configurable absolute-floor and incumbent-delta promotion gates plus a promotion registry. A first smoke-profile cold-start versus teacher-bootstrap comparison exists, but stricter long-run experiment criteria are still informal.
- CPU-only run audit CLI over linear and neural self-play manifests that checks latest benchmark health, capped-game rates, same-opponent benchmark regression from previous best, missing latest benchmark opponents, and trailing promotion failures. It can also enforce opt-in upper bounds on latest collection and benchmark average decision-round length plus process peak RSS, discover pilot manifests with globs, suggest single-run or aggregate multi-run audit thresholds from observed manifest history, overlay named audit-profile status onto side-by-side run comparisons, run a CPU pilot-suite wrapper that repeats seeded smoke pilots before writing and replaying a calibrated audit config with persisted calibration/replay compare artifacts, and launch guarded CPU long-run commands from ready pilot artifacts with wrapper summaries and derived nested audit reports.
...
- Neural iteration can use the shared promotion registry/gate path, and run-level audits can flag obvious benchmark/capped-rate/promotion regressions. Named profiles, pilot-calibrated audit configs, guarded long-run launch wrappers, and derived nested audit reports make current threshold intent and run health explicit, but useful long-run settings still need empirical validation on larger experiments.
- The scripted teacher uses local Showdown dex metadata plus first-pass context heuristics for utility moves, safer switching, max-layer Spikes suppression, and Rapid Spin hazard clearing. It is a bootstrap data source, not the intended long-term policy, and it still lacks deeper hazard planning and sequence planning.
...
`docs/cpu_self_play_roadmap.md` tracks the remaining CPU milestone work. The short version is that the wiring, local pilot evidence, pilot audit calibration, a current standalone teacher-bootstrap checkpoint, and a first smoke-profile cold-start versus teacher-bootstrap comparison are present; the next phase is tightening thresholds and rerunning larger comparisons under stricter guardrails.
