...
- Configurable promotion gate CLI over bootstrap and self-play manifests using benchmark win rate, capped-game rates, and teacher-degradation counters.
...
- Evaluation exists against fixed baselines and historical checkpoints, with configurable promotion gates; long-run experiment criteria are still informal.
...
- Formal long-run benchmark thresholds, regression detection, or checkpoint pool curation.
