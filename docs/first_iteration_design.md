...
`docs/cpu_self_play_roadmap.md` tracks the remaining CPU milestone work. The short version is that the wiring, local pilot evidence, pilot audit calibration, and a current standalone teacher-bootstrap checkpoint are present; the next phase is comparing cold-start versus teacher-bootstrap linear self-play runs under the same guardrails, then tightening thresholds from those artifacts.
