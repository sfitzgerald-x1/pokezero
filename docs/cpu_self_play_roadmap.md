...
2. If choosing between cold-start and teacher-bootstrap, expand shared-opponent benchmark evidence to include promoted historical or static bootstrap opponents; the first direct fixed-baseline plus head-to-head result is mixed.
...
- Ran a direct shared-opponent benchmark at `runs/shared-policy-benchmark-local-20260623/cold-vs-teacher-shared-opponents.json` using `rollout_cli benchmark`, the latest cold-start and teacher-bootstrap checkpoints from the adjusted comparison, 30 games per mirrored matchup, shared seed start `9200001`, shared `random-legal` and `simple-legal` opponents, and candidate head-to-head enabled. The run completed 300 total games in 428.2 seconds, with zero capped games, average decision rounds 62.71, 0.701 games/s, 43.94 decisions/s, and peak RSS 401.09 MB. Against fixed baselines, cold-start scored 47/60 versus `random-legal` and 33/60 versus `simple-legal`; teacher-bootstrap scored 45/60 versus `random-legal` and 32/60 versus `simple-legal` with two ties. In direct candidate head-to-head, teacher-bootstrap beat cold-start 35/60 versus 25/60. This is useful comparable evidence, but it is mixed: cold-start is slightly ahead on fixed baselines while teacher-bootstrap wins the direct head-to-head, so it should not by itself decide the next incumbent.
