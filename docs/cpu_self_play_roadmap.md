...
1. Run the expanded cold-start versus teacher-bootstrap comparison under provisional envelope guardrails.

   A first comparable local run exists, but it used the permissive smoke audit profile to validate the path. A summary-based envelope calibration over that cold-start/bootstrap pair now gives provisional guardrails that keep both early arms passable while still requiring benchmark coverage, fixed-baseline coverage, capped-game health, decision-round ceilings, and RSS health. The next implementation step is to make the long-run wrapper consume a summary-derived runtime audit config, then run the expanded comparison with enough benchmark games to treat benchmark win rate, capped-game rate, average decision rounds, throughput, RSS high-water marks, and promotion outcomes as actionable experiment evidence. Validation fit remains imitation-health only.
...
1. Let `cpu-long-run-plan/run` consume a summary-derived runtime audit config from `cpu-long-run-calibrate`, not only the pilot-generated audit config.
2. Run an expanded cold-start versus teacher-bootstrap comparison with a feasible promotion gate profile and the provisional envelope thresholds from the early comparison.
3. Recalibrate thresholds after the expanded comparison, preferring envelope mode while evidence is thin.
