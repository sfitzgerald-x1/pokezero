...
1. Run the expanded cold-start versus teacher-bootstrap comparison under provisional envelope guardrails.
   ...
   A first comparable local run exists, but it used the permissive smoke audit profile to validate the path. A summary-based envelope calibration over that cold-start/bootstrap pair now gives provisional guardrails that keep both early arms passable while still requiring benchmark coverage, fixed-baseline coverage, capped-game health, decision-round ceilings, and RSS health. The next comparison should persist those thresholds with `cpu-long-run-calibrate --write-config`, pass the config through `cpu-long-run-plan/run --runtime-audit-config`, and use enough benchmark games to treat benchmark win rate, capped-game rate, average decision rounds, throughput, RSS high-water marks, and promotion outcomes as actionable experiment evidence. Validation fit remains imitation-health only.
...
1. Run an expanded cold-start versus teacher-bootstrap comparison with a feasible promotion gate profile and `--runtime-audit-config` pointing at a persisted provisional envelope config.
2. Recalibrate thresholds after the expanded comparison, preferring envelope mode while evidence is thin.
