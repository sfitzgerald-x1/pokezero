...
- A first matched local cold-start versus teacher-bootstrap comparison has completed under the smoke audit profile. It validates the run/report/compare/readiness path for both arms and provides early relative evidence, but it is not yet a calibrated long-run quality threshold.

## What Is Left

The remaining work is less about wiring and more about making the loop empirically usable.

1. Scale the cold-start versus teacher-bootstrap comparison under stricter guardrails.

   A first comparable local run exists, but it used the permissive smoke audit profile to validate the path. The next comparison should use the stronger pilot-derived audit config or a stricter named profile, with enough benchmark games to treat benchmark win rate, capped-game rate, average decision rounds, throughput, RSS high-water marks, and promotion outcomes as actionable experiment evidence. Validation fit remains imitation-health only.
...
- Ran a matched local smoke-profile comparison at `runs/cpu-comparison-local-20260622` using two iterations, 16 collection games per iteration, four workers, `--evaluation-games 20`, `window-size 4`, `feature-count 131072`, and shared seed bands. The cold-start arm completed in 426.8 seconds with two recorded promotions, latest benchmark win rate 0.5583 over 120 latest benchmark games, latest benchmark capped rate 0.0167, latest average decision rounds 68.19, benchmark average decision rounds 68.60, and peak RSS 443.11 MB. The teacher-bootstrap arm started from `runs/scripted-teacher-bootstrap-local-20260622-medium-1/linear-bootstrap.json`, completed in 439.6 seconds with two recorded promotions, latest benchmark win rate 0.5875 over 160 latest benchmark games, zero latest benchmark capped games, latest average decision rounds 60.00, benchmark average decision rounds 58.89, and peak RSS 578.66 MB. `cpu-long-run-report --require-derived-audit`, `cpu-long-run-compare --fail-on-non-passing`, and `cpu-readiness-report --require-ready` all passed for these artifacts. Because the runtime audit source was the smoke profile rather than the pilot-derived audit config, this is path validation plus early relative evidence, not final long-run strength evidence.
