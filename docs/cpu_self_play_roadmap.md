...
2. Finish promotion registry discipline rehearsal for historical opponents.

   Long runs should use `--auto-promote`, a promotion registry, a managed artifact directory, and a required promoted-opponent-pool preflight once enough promoted checkpoints exist. Historical opponents should come from accepted checkpoints, not every raw iteration. The selector now excludes the current checkpoint by resolved `linear:`/`neural:` checkpoint path, so relative launch specs and absolute registry specs agree, and auto-promoted managed artifact copies become the next current policy spec instead of appearing as historical opponents. A full guarded rerun is still needed after the first rehearsal exposed both the selector bug and a tight RSS ceiling.
...
1. Rerun the promoted-pool rehearsal with the fixed current-policy exclusion and decide whether the current RSS ceiling should remain hard, move to warning-only, or be recalibrated from more evidence.
2. If choosing between cold-start and teacher-bootstrap, expand shared-opponent benchmark evidence to include promoted historical opponents and larger pairwise samples. A static bootstrap opponent can also be useful, but it should be interpreted carefully because the teacher-bootstrap arm was initialized from that checkpoint.
