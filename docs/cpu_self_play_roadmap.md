...
- The promotion registry records accepted checkpoints, can copy managed artifacts, verifies registry/checkpoint integrity, feeds promoted opponent pools back into self-play, previews pool selection, excludes the current checkpoint by resolved checkpoint identity rather than raw string equality, and supports recoverable retention archiving.
