...
it `protect` and `engine_world.py:1340` already seeds it) and is withdrawn.
...
> remaining anywhere** ("112 of 119 … 12 by side-symmetry … 7 open" are all stale). The
> "all three attributed causes are **harness fixes**" clause was also wrong: C112's own
> file-location table puts P1 in `leaf.rs`, P2 in `events.rs` and P5 in `engine_world.py` —
> **production**, not harness. Kept rather than deleted because the numbers were published and a
> reader may have quoted them; go to C112 for anything current.
...
| `root_frozen_pack` | n/a (v4 only) | n/a (v4 only) | **V4 k0 feature pack, root-frozen at the leaf.** The 13 pack columns enumerated in `leaf_vs_reality.py::V4_ROOT_FROZEN_PACK_COLUMNS` are written from the ROOT's `observation_metadata` and never re-derived per leaf, so at depth > 0 they describe the root. **Permanent by design** — the hazard credit/expected pair encodes the gen3 grounding rule (Levitate, Flying, unrevealed-slot accounting); re-deriving that rule in Rust is exactly the duplication this design refuses, since a second implementation free to drift is worse than a stale-but-consistent one. Same for the damage ledgers, Truant phase, items-removed credit, choice lock, item-swapped, last-used move and traced ability. **The former "(b) follow-up, not accepted forever" members are GONE from this set** — `NUMERIC_MON_SWITCHED_VS_ACTIVE` / `NUMERIC_MON_STAYED_VS_ACTIVE` are now written from the fold at the leaf; see the `matchup_fold` row below. **A1 (forced recharge) is SPLIT BY SIDE** and is not in this set, because it rides the `CATEGORY_VOLATILE_*` bag as `volatile:mustrecharge` rather than owning a column. **Opponent side: LIVE** — it was the one pack member that did not merely go stale at a leaf but contradicted the *same observation's* action surface, which reads the `MUSTRECHARGE` volatile live to present the forced single "recharge" pseudo-move; `leaf.rs` refreshes `opponent_must_recharge` from the branch's own `volatile_statuses`. **Self side: LIVE.** (Was root-frozen; lifted once the reason for the freeze was gone.) The freeze was correct only because `engine_search.py::_recharging_slots` returned the opponent slot or nothing, never ours, so deriving the self flag from `volatile_statuses` would have written `False` at depth 0 for a genuinely recharging self mon and regressed root/leaf parity. `_recharging_slots` is now **symmetric** — it locks our own slot from the parser's `self_must_recharge`, the same `must_recharge` tracker that feeds `opponent_must_recharge`, published per seat. **Amended:** `self_must_recharge` is published only under the v4 feature-pack schemas, so on v2.2/v3 that lock was absent and the recharge turn was still unsearchable; `_recharging_slots` now UNIONS the tracker with the request's own legal choice set, read off the ungated `action_candidates` metadata (`engine_search.self_recharge_from_action_candidates`). Both are positive proofs about our own seat, so the union cannot manufacture a lock, and `fidelity_gate_events.production_recharging_slots` calls the same function rather than restating it. The root world therefore carries the volatile and that depth-0 regression can no longer happen. The freeze is now **lifted**: `leaf.rs` derives both sides from the branch's own `volatile_statuses`. It had to go rather than merely being allowed to stay — making `_recharging_slots` symmetric meant `engine_search` began BUILDING self-recharge worlds that previously failed closed, so the stale root flag became reachable in production search at depth > 0. The strongest argument for the lift is one this row did not make: `leaf.rs` already read our own side LIVE — `let recharging = self_side.volatile_statuses.contains(&...MUSTRECHARGE)` in the action-surface block, cited by expression because the line number drifted twice to present the forced "recharge" pseudo-move, so the self side carried exactly the action-surface-vs-volatile-bag self-contradiction cited above as the reason the OPPONENT side had to be live. Verified: `leaf_root_parity` stays at `diverged 0` (depth-0 parity, the freeze's whole rationale), and the derivation is pinned by `tests/test_leaf_self_recharge_derivation.py`. **Correction, recorded rather than quietly dropped:** this row previously said the asymmetry meant "the production root world lets our recharging mon pick any move". That is wrong. Showdown sets `trapped: true` on a recharge request, so `engine_world.py` rejected those worlds outright as `self_request_state_unsupported` and search failed closed to the fallback — our recharge turns were UNSEARCHABLE, not mis-searched. The free-choice harm was real only in the rare sub-case where the foe independently traps us. **Gate caveat, now RETIRED:** the four gates used to derive `recharging` for BOTH slots from the recorded chosen candidate, seeding each gate's world from the very thing it was checking; they now use `fidelity_gate_events.production_recharging_slots`, which mirrors `_recharging_slots` and reads the parser tracker, so they can catch a bad self-side write rather than ratify it. Independence from the recorded action is what makes catching possible; symmetry is what keeps the two self-recharge boundaries measured. **Shown end-to-end** in `reports/c141_recharge_gate_injection_proof.md`: a wrong self-side write compiled into `leaf.rs` (wired to the opponent's volatile) takes `leaf_root_parity` from `diverged 0 / EXIT=0` to `diverged 5 / EXIT=1` at an unchanged denominator, and back to a byte-identical baseline on revert. **Read its limits section before quoting it:** the pre-task-4 gates also caught 4 of those 5, and additionally reported one spurious opponent-side divergence — because the recorded action and the parser tracker disagree on exactly one row of this corpus, and it is opponent-side. A corpus with no self-side disagreement cannot discriminate the two derivations on the self side, which is why `tests/test_recharge_gate_derivation.py` uses fixtures. |
| `meanlook_trap` | n/a (derived) | n/a (derived) | **V3 change 8 (NUMERIC_MEANLOOK_TRAP), LIVE at the leaf — never root-frozen.** Recorded here because it was ROOT-FROZEN BY OMISSION until the move trap was routed into the world payload, and the omission was invisible: `TRAPPED` is absent from `leaf.rs::VOLATILE_MAP` (the parser records the move trap in its own `meanlook_trap` tracker, not in the TRACKED_VOLATILES bag), so `tracked_volatiles` could not carry it, nothing in `leaf.rs` wrote it, and `encoder.rs` read `{prefix}_meanlook_trap` out of the ROOT's metadata for every v3 AND v4 layout. Harmless only while unreachable — a move-trapped root was refused as `self_request_state_unsupported`, so no trapped position entered a search. **Same defect shape as A1's self side** (`root_frozen_pack` above): the fix that makes the class searchable is the fix that makes the stale flag reachable at depth > 0. And the same clinching argument applies — the ACTION SURFACE is already live, since switch options come from the engine's `get_all_options` honouring `Side::trapped` (gen3/state.rs:493), so a frozen bag would contradict its own observation. `leaf.rs` therefore derives BOTH sides from the branch's own `volatile_statuses.contains(TRAPPED)`, beside the MUSTRECHARGE write. **Not added to `V4_ROOT_FROZEN_PACK_COLUMNS`, and that list is an EXCUSE list rather than a documentation list** — `leaf_vs_reality.classify` returns `root_frozen_pack` for its members, and `gate_exit_code` fails only on `state`/`turn` (plus the matchup allowance). Adding a now-DERIVED must-match column to it would move it into a non-gating class and suppress exactly the divergences the derivation exists to expose. It is also a v3 column rather than a v4 pack member. The branch value genuinely moves — the engine drops TRAPPED when the trapper leaves the field and carries it through a Baton Pass (gen3/state.rs:825 `TRAPPED => baton_passing`). **ONE-OFF GATE ARTIFACT, expected and not a regression:** because the crate now derives the flag from the world instead of passing the root's metadata through, a corpus row recorded BEFORE the `meanlookTrap` payload key existed has a root that says `true` and a world built free, so the crate writes `false` at DEPTH 0. Reproduced with the real `LeafEncoder`. Production cannot hit it (its payloads carry the key), but `leaf_root_parity` / `leaf_vs_reality` run against a pre-PR corpus containing a Mean Look boundary will report a new `state`-class defect and exit 1. Re-record the corpus before reading those gates. **Known producer gap, not a leaf gap:** the PARSER clears `meanlook_trap` on any switch including a Baton Pass, which gen3 does not do — `data/mods/gen4/conditions.ts` re-declares BOTH `trapped` and `trapper` with `noCopy: false` and gen3 inherits gen4, so there are TWO broken directions (the VICTIM passing, whose receiver inherits `trapped`; and the TRAPPER passing, whose receiver inherits `trapper` so the linked trap survives). Either way a webbed-then-passed root arrives with the flag already False. Fail-closed on the SELF side only — `_require_world_reproduces_trap` inspects our own side, so a missed self trap is refused, while a missed OPPONENT trap has no such check and is built free. Tracked separately from this row; both directions are pinned in `tests/test_move_trap_payload_wiring.py`. |
...
`DEFECT-CLASS divergent boundaries: 0`. With boundaries encoding again,
`block_of` raised `TypeError` — it had gained a parameter the call site never
passed, because the classification path had never once executed. Regenerate
...
`root_frozen_pack` and `matchup_fold` are the two classes that exist at v4
specifically. The first is accepted-forever by design; the second is not an
excuse class at all -- it is gated, on magnitude rather than count, and the pair
it covers used to be the "tracked follow-up" half of `root_frozen_pack`.
