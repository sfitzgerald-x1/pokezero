...
def _observation_with_self_hp_fraction(*, legal_action: int = 0, hp_fraction: float = 1.0) -> PokeZeroObservationV0:
    ...
    return _observation_with_numeric_feature(
...
def _observation_with_opponent_hp_fraction(
    *,
    legal_action: int = 0,
    hp_fraction: float = 1.0,
) -> PokeZeroObservationV0:
    token_index = FIELD_TOKEN_COUNT + SELF_POKEMON_TOKEN_COUNT
    return _observation_with_numeric_feature(
        legal_action=legal_action,
        token_index=token_index,
        feature_index=0,
...
def _observation_with_self_numeric_feature(
    *,
    legal_action: int = 0,
    feature_index: int,
    value: float,
...
def _observation_with_numeric_feature(
    *,
    legal_action: int,
    token_index: int,
    feature_index: int,
    ...
    numeric_features[token_index][feature_index] = value
...
class ReplayBranchingUnitTest(unittest.TestCase):
    ...
    def test_start_override_consistency_accepts_small_opponent_hp_fraction_drift(self) -> None:
        env = ObservationReplayEnv(
            (("p1",),),
            _observation_with_opponent_hp_fraction(hp_fraction=0.514),
        )
        start_override = BattleStartOverride(
            player_teams={"p1": "Charizard||||Tackle|||||||", "p2": "Xatu||||Psychic|||||||"}
        )
        ...
        replay_action_rounds(
            ...
            action_rounds=(
                ReplayActionRound(
                    ...
                    actions={"p1": 0},
                    expected_observations={
                        "p1": _observation_with_opponent_hp_fraction(hp_fraction=0.500),
                    },
                ),
            ),
            start_override=start_override,
    ...
    def test_start_override_consistency_rejects_non_hp_numeric_drift_with_tolerance(self) -> None:
        env = ObservationReplayEnv(
            (("p1",),),
            _observation_with_self_numeric_feature(feature_index=1, value=0.514),
        )
        start_override = BattleStartOverride(
            player_teams={"p1": "Charizard||||Tackle|||||||", "p2": "Xatu||||Psychic|||||||"}
        )
            ...
            replay_action_rounds(
                ...
                action_rounds=(
                    ReplayActionRound(
                        ...
                        actions={"p1": 0},
                        expected_observations={
                            "p1": _observation_with_self_numeric_feature(feature_index=1, value=0.500),
                        },
                    ),
                ),
                start_override=start_override,
