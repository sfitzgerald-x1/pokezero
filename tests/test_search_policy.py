...
from pokezero.search import RootPUCTSearchTiming, puct_branch_search, puct_branch_search_group
...
class RootPUCTSearchPolicyTest(unittest.TestCase):
    ...
    def test_root_puct_policy_expands_start_override_samples_as_weighted_scenarios(self) -> None:
        ...
        self.assertEqual(metadata["root_puct_start_override_attempts"], 1)
        self.assertEqual(metadata["root_puct_start_override_attempts_used"], 2)
        self.assertEqual(metadata["root_puct_start_override_samples_per_scenario"], 2)
        self.assertEqual(metadata["root_puct_opponent_action_scenarios_generated"], 2)
        self.assertEqual(metadata["root_puct_opponent_action_scenario_count"], 2)
        self.assertEqual(metadata["root_puct_opponent_action_groups_generated"], 1)
        self.assertEqual(metadata["root_puct_opponent_action_groups_used"], 1)
        self.assertEqual(metadata["root_puct_opponent_action_groups_skipped"], 0)
        self.assertEqual(metadata["root_puct_opponent_action_groups_unsearched"], 0)
        self.assertEqual(
            metadata["root_puct_opponent_action_scenarios"],
            [
                {"label": "stay-in/belief-sample-1", "weight": 0.5, "actions": {"p2": 0}},
                {"label": "stay-in/belief-sample-2", "weight": 0.5, "actions": {"p2": 0}},
            ],
        )
    ...
    def test_root_puct_fallback_preserves_completed_initial_value_batches(self) -> None:
        branch_envs: list[NonterminalDirectSnapshotStartOverrideEnv] = []
        batch_calls = 0
        ...
        def batch_values(histories: tuple[tuple[PokeZeroObservationV0, ...], ...]) -> tuple[float, ...]:
            return tuple(0.0 for _history in histories)
        ...
        def scenario_planner(
            ...
            del context, rng
            return (
                OpponentActionScenario(actions={"p2": 0}, label="first"),
                OpponentActionScenario(actions={"p2": 1}, label="second"),
            )

        def fail_after_first_group(*args, **kwargs):
            nonlocal batch_calls
            if batch_calls:
                raise RuntimeError("forced second grouped-search failure")
            batch_calls += 1
            return puct_branch_search_group(*args, **kwargs)
        ...
        policy = RootPUCTSearchPolicy(
            ...
            value_batch_fn=batch_values,
            prior_fn=lambda _history: (0.5, 0.5) + (0.0,) * (ACTION_COUNT - 2),
            opponent_action_scenario_planner=scenario_planner,
            fallback_policy=FixedPolicy(1, policy_id="raw"),
            allow_fallback=True,
            cpuct=0.0,
        ...
        self.assertEqual(decision.metadata["root_puct_cross_world_initial_value_batch_count"], 1)
        self.assertEqual(
            decision.metadata["root_puct_cross_world_initial_value_batch_world_count"],
            2,
        )
    ...
    def test_root_puct_policy_skips_invalid_shared_start_override_sample_once_per_slot(self) -> None:
        ...
        self.assertEqual(metadata["root_puct_start_override_sources_used"], 2)
        self.assertEqual(metadata["root_puct_start_override_shared_samples"], 2)
        self.assertEqual(metadata["root_puct_start_override_shared_samples_accepted"], 1)
    ...
    def test_root_puct_policy_retries_shared_start_override_sample_slots(self) -> None:
        branch_envs: list[RejectingStartOverrideOutcomeEnv] = []
        planner_calls: list[tuple[str, int]] = []
        calls_by_sample: dict[int, int] = {}
        ...
        self.assertEqual(metadata["root_puct_start_override_shared_samples_accepted"], 2)
        self.assertEqual(metadata["root_puct_start_override_shared_samples_rejected"], 0)
        self.assertEqual(len(branch_envs[0].start_overrides), 6)

    def test_root_puct_policy_fallback_reports_all_shared_start_override_samples_rejected(self) -> None:
        branch_envs: list[RejectingStartOverrideOutcomeEnv] = []
        planner_calls: list[tuple[str, int]] = []
        ...
        self.assertEqual(metadata["fallback_policy_id"], "fallback-fixed")
        self.assertEqual(
            planner_calls,
            [
                ("stay-in/belief-sample-1", 0),
        ...
        self.assertEqual(metadata["root_puct_start_override_shared_samples_rejected"], 2)
        self.assertEqual(metadata["root_puct_opponent_action_scenarios_generated"], 4)
    ...
    def test_root_puct_policy_fallback_keeps_partial_belief_world_attempt_count(self) -> None:
        branch_envs: list[RejectingStartOverrideOutcomeEnv] = []
        planner_calls = 0
        ...
        def start_override_planner(
            ...
            planner_calls += 1
            if planner_calls == 2:
                raise RuntimeError("belief-world planner crashed")
        ...
        self.assertTrue(decision.metadata["root_puct_fallback"])
        self.assertIn("belief-world planner crashed", decision.metadata["root_puct_fallback_reason"])
        self.assertEqual(planner_calls, 2)
        timing = decision.metadata["root_puct_timing"]
        self.assertEqual(timing["belief_world_materialization_count"], 2)
        self.assertGreater(timing["belief_world_materialization_seconds"], 0.0)
    ...
    def test_root_puct_policy_caps_opponent_actions_not_belief_samples_after_skips(self) -> None:
        ...
        def scenario_planner(context: PolicyContext, rng: random.Random) -> tuple[OpponentActionScenario, ...]:
            del context, rng
            return (
                OpponentActionScenario(actions={"p2": 0}, weight=0.8, label="first-action"),
                OpponentActionScenario(actions={"p2": 1}, weight=0.2, label="second-action"),
            )
        ...
        def start_override_planner(
            ...
            del context, rng
            planner_calls.append((scenario.label, scenario_index))
            species = "Badmon" if scenario.label == "first-action/belief-sample-1" else "Xatu"
        ...
        self.assertEqual(
            planner_calls,
            [
                ("first-action/belief-sample-1", 0),
                ("first-action/belief-sample-2", 1),
            ],
        )
        self.assertEqual(branch_envs[0].rejected_start_overrides, 1)
        self.assertEqual(branch_envs[0].all_step_calls, [
        ...
        self.assertEqual(metadata["root_puct_start_override_sources_used"], 1)
        self.assertEqual(metadata["root_puct_start_override_attempts_used"], 2)
        self.assertEqual(metadata["root_puct_opponent_action_scenarios_generated"], 4)
        ...
        self.assertEqual(metadata["root_puct_opponent_action_groups_used"], 1)
        self.assertEqual(metadata["root_puct_opponent_action_groups_skipped"], 0)
        self.assertEqual(metadata["root_puct_opponent_action_groups_unsearched"], 1)
        self.assertEqual(
            metadata["root_puct_opponent_action_scenarios"],
            [{"label": "first-action/belief-sample-2", "weight": 1.0, "actions": {"p2": 0}}],
        )
        self.assertEqual(
            metadata["root_puct_opponent_action_skipped_scenarios"],
            [
                {
                    "label": "first-action/belief-sample-1",
                    "weight": 0.4,
                    "actions": {"p2": 0},
                    "reason": (
                        "start override does not reproduce recorded replay prefix observations "
    ...
    def test_root_puct_policy_skips_reason_bearing_missing_start_override_source(self) -> None:
        branch_envs: list[ImmediateOutcomeEnv] = []
    ...
    def test_root_puct_policy_visits_selection_mode_uses_most_visited_branch(self) -> None:
        ...
        self.assertNotIn("root_puct_root_visit_budget_mode", step.metadata)

    def test_root_puct_policy_resolves_adaptive_visit_budget_after_mandatory_sweep(self) -> None:
        selector_calls = []

        def selector(context: PolicyContext, budget_context) -> int:
            selector_calls.append((context, budget_context))
            return 5

        selector.selector_id = "test-adaptive"  # type: ignore[attr-defined]
        policy = RootPUCTSearchPolicy(
            env_factory=lambda: ImmediateOutcomeEnv(label="branch"),
            rollout_config=RolloutConfig(max_decision_rounds=3),
            value_fn=lambda history: 0.0,
            ...
            cpuct=2.0,
            root_visit_budget=2,
            root_visit_budget_selector=selector,
        )
        ...
        selector_context, budget_context = selector_calls[0]
        self.assertEqual(selector_context.player_id, "p1")
        self.assertEqual(budget_context.configured_root_visit_budget, 2)
        ...
        self.assertEqual(step.metadata["root_puct_configured_root_visit_budget"], 2)
        self.assertEqual(step.metadata["root_puct_effective_root_visit_budgets"], (5,))
        self.assertEqual(step.metadata["root_puct_visit_budget_contexts"][0]["value_margin"], 2.0)

        entropy_selector = EntropyMarginVisitBudgetSelector(
            contested_extra_visits=3,
            minimum_policy_entropy=0.2,
        )
        margin_selector = EntropyMarginVisitBudgetSelector(
            contested_extra_visits=3,
            maximum_value_margin=2.0,
        )
    ...
    def test_puct_visit_budget_resolver_rejects_budgets_below_mandatory_sweep(self) -> None:
        with self.assertRaisesRegex(ValueError, "root_visit_budget_resolver.*legal root actions"):
            puct_branch_search(
                ...
                root_visit_budget=2,
                root_visit_budget_resolver=lambda budget_context: 1,
            )

    def test_puct_visit_budget_resolver_can_stop_after_sweep_with_base_priors(self) -> None:
        budget_contexts = []

        def resolver(budget_context):
            budget_contexts.append(budget_context)
            return None
        ...
        search = puct_branch_search(
            ...
            budget_action_priors=(0.9, 0.1) + (0.0,) * (ACTION_COUNT - 2),
            root_visit_budget=1,
            root_visit_budget_resolver=resolver,
        )

        self.assertEqual(search.total_visits, 2)
        self.assertIsNone(search.root_visit_budget)
    ...
    def test_root_puct_policy_can_use_bounded_leaf_rollouts_for_branch_values(self) -> None:
        ...
        self.assertEqual(timing["value_evaluation_count"], 0)
        self.assertEqual(timing["rollout_tail_count"], 2)
        self.assertEqual(timing["policy_value_evaluation_count"], 1)
        self.assertEqual(len(branch_envs), 1)
        self.assertTrue(branch_envs[0].closed)
        self.assertEqual(
    ...
    def test_root_puct_policy_rejects_invalid_start_override_samples_per_scenario(self) -> None:
        with self.assertRaisesRegex(ValueError, "start_override_samples_per_scenario"):
            RootPUCTSearchPolicy(
                env_factory=lambda: ImmediateOutcomeEnv(label="branch"),
                rollout_config=RolloutConfig(max_decision_rounds=3),
                value_fn=lambda history: 0.0,
                prior_fn=lambda history: (1.0,) + (0.0,) * (ACTION_COUNT - 1),
                start_override_samples_per_scenario=0,
            )
        with self.assertRaisesRegex(ValueError, "start_override_samples_per_scenario"):
            RootPUCTSearchPolicy(
                env_factory=lambda: ImmediateOutcomeEnv(label="branch"),
