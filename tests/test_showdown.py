...
from pokezero.showdown import (
    ...
    NUMERIC_MOVE_STAB,
    NUMERIC_MOVE_TARGET_EFFECTIVENESS,
    NUMERIC_OPP_FUTURE_SIGHT,
    NUMERIC_SELF_FUTURE_SIGHT,
    NUMERIC_SELF_HP_COST,
    NUMERIC_STAY_INCOMING_EFFECTIVENESS,
    NUMERIC_STAY_INCOMING_DAMAGE,
    NUMERIC_STAY_INCOMING_POWER,
    NUMERIC_SWITCH_INCOMING_EFFECTIVENESS,
    NUMERIC_SWITCH_INCOMING_DAMAGE,
    NUMERIC_SWITCH_INCOMING_POWER,
    NUMERIC_TOXIC_STAGE,
    NUMERIC_TURN_COUNT,
...
def _phase2_fake_dex() -> ShowdownDex:
    ...
    return ShowdownDex(
        moves={"flamethrower": flamethrower, "psychic": psychic},
        species={"charizard": charizard, "xatu": xatu, "snorlax": snorlax},
        type_chart={
            "flying": {"fire": 1},  # fake chart for deterministic test coverage
            "normal": {"psychic": 1},
        },
    )
...
class ShowdownReplayNormalizationTest(unittest.TestCase):
    ...
    def test_observation_encodes_public_belief_summary_features(self) -> None:
        ...
        self.assertEqual(observation.metadata["opponent_active_revealed_moves"], ["Psychic"])
        self.assertEqual(observation.metadata["opponent_active_possible_moves"], ["psychic", "thunderwave", "wish"])
...
class Phase2DynamicStateTest(unittest.TestCase):
    ...
    def test_move_matchup_features_on_move_action_token(self) -> None:
        state = self._replay_with([])
        observation = observation_from_player_state(
            state, category_vocab=_TEST_VOCAB, dex=_phase2_fake_dex()
        )
        move_token = self.ACTION_OFFSET  # Flamethrower by Charizard into Xatu.

        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_MOVE_TARGET_EFFECTIVENESS],
            0.5,  # fake chart: Fire is 2x into Flying, normalized by /4
        )
        self.assertAlmostEqual(observation.numeric_features[move_token][NUMERIC_MOVE_STAB], 1.0)
        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_MOVE_EXPECTED_POWER],
            0.95,  # 95 BP * 2x * STAB / 300
        )
        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_MOVE_ESTIMATED_DAMAGE],
            1.0,  # approximate Flamethrower damage into already-damaged Xatu clamps at a KO.
        )
        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_STAY_INCOMING_EFFECTIVENESS],
            0.25,  # fake chart: opponent Xatu's Psychic is neutral into Charizard, normalized by /4
        )
        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_STAY_INCOMING_POWER],
            0.45,  # 90 BP * neutral * STAB / 300
        )
        self.assertAlmostEqual(
            observation.numeric_features[move_token][NUMERIC_STAY_INCOMING_DAMAGE],
            0.37145283321006894,
        )

    def test_switch_matchup_feature_on_switch_action_token(self) -> None:
        state = self._replay_with([])
        observation = observation_from_player_state(
            state, category_vocab=_TEST_VOCAB, dex=_phase2_fake_dex()
        )
        snorlax_switch_token = self.ACTION_OFFSET + 4

        self.assertAlmostEqual(
            observation.numeric_features[snorlax_switch_token][NUMERIC_SWITCH_INCOMING_EFFECTIVENESS],
            0.5,  # fake chart: opponent Xatu's revealed Psychic is 2x into Normal, normalized by /4
        )
        self.assertAlmostEqual(
            observation.numeric_features[snorlax_switch_token][NUMERIC_SWITCH_INCOMING_POWER],
            0.9,  # 90 BP * 2x * STAB / 300
        )
        self.assertAlmostEqual(
            observation.numeric_features[snorlax_switch_token][NUMERIC_SWITCH_INCOMING_DAMAGE],
            0.4059514893552685,
        )

    def test_switch_target_status_is_encoded_on_switch_action_token(self) -> None:
        lines = [
            line.replace(
                '"ident":"p2b: Snorlax","details":"Snorlax, L78","condition":"100/100","active":false',
                '"ident":"p2b: Snorlax","details":"Snorlax, L78","condition":"100/100 par","active":false',
            )
            for line in fixture_lines("p2_seat_replay.txt")
        ]
        replay = parse_showdown_replay(lines, battle_id="battle-gen3randombattle-1")
        state = normalize_for_player(replay, player_id="agent", player_name="PokeZeroBot")
        observation = observation_from_player_state(
        ...
        self.assertEqual(
            observation.categorical_ids[snorlax_switch_token][CATEGORY_MOVE_EFFECT],
            stable_category_id("status:par"),
        )
