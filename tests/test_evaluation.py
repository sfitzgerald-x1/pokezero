...
""",
    ...
    def test_eval_cli_cpu_long_run_plan_custom_runtime_audit_config_overrides_long_run_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            pilot_root, pilot_audit_config_path, _, _ = write_ready_cpu_long_run_pilot(
                temp_path,
            ...
            runtime_audit_config_path = temp_path / "summary-envelope-audit-config.json"
            write_json(
                runtime_audit_config_path,
                run_audit_config_payload(
                    RunAuditConfig(
            ...
            with patch("sys.stdout", new_callable=io.StringIO) as stdout:
                exit_code = eval_cli_main(
                    [
                        ...
                        "--json",
                        "--run-dir",
                        str(temp_path / "long-run-with-summary-config"),
                        "--initial-policy",
                        "random-legal",
                        "--profile",
                        "long-run",
    ...
    def test_eval_cli_cpu_long_run_plan_rejects_custom_runtime_audit_config_with_explicit_other_source(self) -> None:
        for runtime_audit_source in ("profile", "pilot-audit-config"):
            with self.subTest(runtime_audit_source=runtime_audit_source):
                with tempfile.TemporaryDirectory() as temp_dir:
                    temp_path = Path(temp_dir)
                    config_path = temp_path / "summary-envelope-audit-config.json"
                    write_json(config_path, run_audit_config_payload(RunAuditConfig()))
                ...
                self.assertEqual(exit_code, 1)
                self.assertIn(
                    "--runtime-audit-config can only be combined with --runtime-audit-source auto or runtime-audit-config",
                    stderr.getvalue(),
                )
    ...
    def test_eval_cli_cpu_long_run_plan_smoke_gate_with_pilot_audit_config_enforces_pilot_floor(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            pilot_root, audit_config_path, _, _ = write_ready_cpu_long_run_pilot(
                temp_path,
                audit_config_min_latest_benchmark_games=1_000,
            )
            write_json(
                audit_config_path,
                run_audit_config_payload(
    ...
    def test_eval_cli_cpu_long_run_report_recomputes_custom_runtime_config(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            run_root = temp_path / "long-run"
            summary_path = run_root / "cpu-long-run-run-summary.json"
            pilot_audit_config_path = temp_path / "pilot-audit-config.json"
            runtime_audit_config_path = temp_path / "summary-envelope-audit-config.json"
            write_manifest(run_root / "manifest.json", selfplay_manifest())
            write_json(
                pilot_audit_config_path,
                run_audit_config_payload(
                    RunAuditConfig(
            ...
            summary["recipe"]["run_dir"] = str(run_root)
            summary["recipe"]["profile"] = "smoke"
            summary["recipe"]["audit_config_path"] = str(pilot_audit_config_path)
            summary["recipe"]["runtime_audit_source"] = "runtime-audit-config"
            summary["recipe"]["runtime_audit_config_path"] = str(runtime_audit_config_path)
            summary["recipe"]["runtime_audit_profile"] = None
            summary["recipe"]["evaluation_games"] = 11
            write_json(summary_path, summary)
        ...
        self.assertFalse(payload["derived_audit_requirement_passed"])
        self.assertTrue(runtime_audit["available"])
        self.assertEqual(runtime_audit["source"], "runtime-audit-config")
        self.assertEqual(runtime_audit["audit_config_path"], str(runtime_audit_config_path))
        self.assertEqual(runtime_audit["recorded_evaluation_games"], 11)
        self.assertEqual(
            runtime_audit["post_iteration_command_flags"],
            [
                "--evaluation-games",
        ...
        self.assertEqual(report["audit_source"], "runtime-audit-config")
    ...
    def test_eval_cli_cpu_long_run_report_classifies_all_promotion_strength_check_names(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_root = Path(temp_dir) / "long-run"
            summary = cpu_long_run_summary(status="passed")
            summary["recipe"]["run_dir"] = str(run_root)
        ...
        self.assertEqual(exit_code, 2)
        derived_report = payload["derived_run_report"]
        self.assertTrue(derived_report["runtime_health_passed"])
        self.assertFalse(derived_report["promotion_strength_passed"])
        self.assertEqual(derived_report["runtime_health_failed_checks"], [])
        self.assertEqual(
            derived_report["promotion_strength_failed_checks"],
            [
                "benchmark_win_rate_drop_by_opponent",
                "consecutive_promotion_failures",
                "latest_promotion_recorded",
    ...
    def test_eval_cli_cpu_long_run_report_require_derived_audit_fails_when_manifest_missing(self) -> None:
        ...
        self.assertFalse(payload["derived_run_report"]["available"])
        self.assertEqual(payload["derived_run_report"]["error"], "manifest_not_found")
        self.assertIsNone(payload["derived_run_report"]["runtime_health_passed"])
        self.assertIsNone(payload["derived_run_report"]["promotion_strength_passed"])
    ...
    def test_eval_cli_cpu_long_run_compare_json_summarizes_wrapper_health(self) -> None:
        ...
        self.assertEqual(payload["entries"][0]["runtime_audit_resolved_source"], "profile")
    ...
    def test_eval_cli_cpu_long_run_compare_json_includes_config_backed_runtime_flags(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            ...
            summary["recipe"]["runtime_audit_source"] = "pilot-audit-config"
            summary["recipe"]["runtime_audit_config_path"] = str(audit_config_path)
            summary["recipe"]["evaluation_games"] = 21
            write_json(run_root / "cpu-long-run-run-summary.json", summary)
        ...
        self.assertEqual(exit_code, 0)
        self.assertTrue(entry["runtime_audit_available"])
        self.assertEqual(entry["runtime_audit_source"], "pilot-audit-config")
        self.assertEqual(entry["runtime_audit_resolved_source"], "pilot-audit-config")
        self.assertEqual(entry["runtime_audit_config_path"], str(audit_config_path))
        self.assertEqual(entry["runtime_audit_recorded_evaluation_games"], 21)
        self.assertEqual(
            entry["runtime_audit_command_flags"],
            ["--evaluation-games", "21", "--audit-after-iteration", "--audit-config", str(audit_config_path)],
        )
        self.assertNotIn("config", entry["runtime_audit"])
    ...
    def test_eval_cli_cpu_long_run_compare_text_prints_unavailable_runtime_audit_flags(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            run_root = temp_path / "run-0001"
            missing_config_path = temp_path / "missing-audit-config.json"
            ...
            summary["recipe"]["runtime_audit_source"] = "pilot-audit-config"
            summary["recipe"]["runtime_audit_config_path"] = str(missing_config_path)
            summary["recipe"]["evaluation_games"] = 8
            write_json(run_root / "cpu-long-run-run-summary.json", summary)

            with patch("sys.stdout", new_callable=io.StringIO) as stdout:
                exit_code = eval_cli_main(["cpu-long-run-compare", str(run_root)])

        output = stdout.getvalue()
        self.assertEqual(exit_code, 0)
        self.assertIn("runtime_audits:", output)
        self.assertIn("available=no", output)
        self.assertIn(f"config={missing_config_path}", output)
        self.assertIn("--evaluation-games 8 --audit-after-iteration --audit-config", output)
    ...
    def test_eval_cli_cpu_readiness_report_treats_missing_supplied_registry_as_error(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            missing_registry = Path(temp_dir) / "missing-promotions.json"
            with patch("sys.stdout", new_callable=io.StringIO) as stdout:
                exit_code = eval_cli_main(
                    ["cpu-readiness-report", "--promotion-registry", str(missing_registry), "--json"]
                )
            payload = json.loads(stdout.getvalue())
        ...
        self.assertEqual(payload["error_count"], 1)
        items = {item["name"]: item for item in payload["items"]}
        self.assertEqual(items["promoted_opponent_pool_ready"]["reasons"], ["promotion_registry_unreadable"])
        self.assertIn("does not exist", items["promoted_opponent_pool_ready"]["evidence"]["error"])

    def test_eval_cli_cpu_readiness_report_enforces_pilot_calibration_requirements_without_duplicate_reasons(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_root = Path(temp_dir) / "pilots"
            summary = cpu_pilot_summary(status="failed", failed_step_index=2)
            summary["recipe"]["audit_config_path"] = str(run_root / "pilot-audit-config.json")
            summary["recipe"]["calibration_output_path"] = str(run_root / "pilot-calibration-compare.json")
            summary["recipe"]["replay_output_path"] = str(run_root / "pilot-audit-replay.json")
            write_json(run_root / "cpu-pilot-suite-summary.json", summary)
        ...
        self.assertEqual(exit_code, 2)
        items = {item["name"]: item for item in payload["items"]}
        reasons = items["pilot_suite_ready"]["reasons"]
        self.assertIn("suite_status_not_passed", reasons)
        self.assertIn("audit_config_unavailable_for_calibration_requirements", reasons)
        self.assertNotIn("pilot_suite_not_passed", reasons)
        self.assertNotIn("audit_config_calibration_requirements_not_passed", reasons)

    def test_eval_cli_cpu_readiness_report_surfaces_failed_long_run_derived_audit_checks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_root = Path(temp_dir) / "long-run"
            summary = cpu_long_run_summary(status="passed")
            report = long_run_derived_report()
            ...
            with patch("sys.stdout", new_callable=io.StringIO) as stdout:
                exit_code = eval_cli_main(
                    ["cpu-readiness-report", "--long-run-summary", str(run_root), "--require-ready", "--json"]
                )
            payload = json.loads(stdout.getvalue())
        ...
        items = {item["name"]: item for item in payload["items"]}
        self.assertEqual(
            items["long_run_derived_audit_ready"]["reasons"],
            ["derived_audit_failed:latest_benchmark_win_rate"],
        )
