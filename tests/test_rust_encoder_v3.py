...
# V3-era sample, deliberately NOT the primary v4 one: the V3 parity fixture needs the TRANSITION token block, which V4 does not have
# (`observation.v4` is 23 tokens with `transition_token_count == 0`, so re-encoding a v4
# row at v3 leaves every NUMERIC_TT_* column flat and the coverage assertions below fail
# for want of a fixture rather than for want of parity). Both samples are regenerated by
# the same command at different --observation-schema values; both are provenance-clean.
