...
> `tests/data/c154_unreachable_readjudication.json`. Nothing was closed and nothing opened.
...
`tests/data/c154_unreachable_readjudication.json` and `tests/test_unreachable_readjudication.py`
...
- **Mechanized, and every demonstration is re-derived from source on every run** — that is the
  ...
  one loud failure. Battery: **12 mutations applied, 12 caught**, enumerated in the pin's
  docstring. ⚠ **Number 8 survived the first version** — a retracted sentence re-inserted
  hard-wrapped *and lower-cased* walked past a guard that already normalised whitespace, which is
  C153's own mutation 24 defeating a guard written by someone who had just read about it. No §3
  row was added or removed, so §3 stays at **82** and §4 still considers 27 candidates.
