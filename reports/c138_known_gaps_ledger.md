...
| **G37c** | **The headline attract measurement is in a file no CI step runs.** `tests/test_instruction_event_mapping.py::test_every_attract_empty_tail_ambiguity_is_resolved` is the nine-shape, zero-refusal measurement that the immobilizer-marker change rests on, and `.github/workflows/engine-fidelity-gates.yml` lists that file in its path FILTERS but no step EXECUTES it. Pre-existing for the file; the marker change is what moved a headline claim into it. Mitigated, not closed: the equivalent Rust sweep (`no_branch_of_an_attracted_or_paralyzed_fan_refuses_any_more`) IS CI-named and count-gated, so the property is protected even though this file's version of it is not. | R/X | n/a — test-coverage gap. | n/a |
