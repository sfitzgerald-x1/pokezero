...
rather than analogy: the two names C152 refuted fire at 3 and 27 per 1,000 games, so this census
would have expected ~24 and ~216 of them. **A negative in the same incidence class as the two that
fell cannot have survived this sample by chance.** What it does *not* exclude is anything below
those bounds, anything behind a non-default flag, and the five entries §6 of the C153 report names
as unreachable by this instrument.
...
- **The reachability check must survive RENDERING, and that is now machine-checked.** Added
  ...
  cell on all **81** gap rows — 80 until C152 added G33c, and the pin is an exact row
...
- ✅ **AND THAT RULE NOW HAS AN INSTRUMENT — added 2026-08-08 (C153), because it did not.**
  ...
  **25.8×** the two windows, on the shipping build. Every entry of a derived 61-name inventory
  ...
  - **Name what the instrument cannot reach, beside the zero.** Five entries carry a
    `census_cannot_reach` note — two rest-sleep legacy canaries whose designed value *is* zero, and
    three payload fields the differential never constructs. A zero from an instrument that could
...
- **H22 added**, taking §3 to **79**: `classify_divergence` leaves 23.7 % of banded divergences in
  the bucket its own docstring forbids.
...
- **Mechanized**, and the battery is the argument: 12 mutations applied, **12 caught**. Two of the
  twelve were written expecting a pass and got one — a closure pin that read the artifact's own
