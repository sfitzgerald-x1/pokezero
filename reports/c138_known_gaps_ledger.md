...
| **G38** | **Sleep Talk's unnamed callee: the largest world-level refusal channel.** `events.rs` `unrenderable_family_at`; era 59 measured `sleeptalk_called_unidentified:ambiguous_unrenderable` at 8,149 world failures, 51.6 % of the abort channel. Five of the six allowlist entries have **no fixture** ("admitted on a structural argument"). ⚠ **PARTIAL DISPOSITION — the `none_matched:shape_length` sub-channel is CLOSED at the engine, not at the renderer.** `generate_instructions_from_move` emitted the turn-start `damage_dealt` carry-over reset TWICE for every Sleep Talk turn carrying a non-default `damage_dealt`: the Sleep Talk block reverses `incoming_instructions` before recursing, restoring the pre-reset carry-over, so the callee's own call re-entered the same opening and pushed the reset again. Because `consume_move_prelude` eats every leading damage-dealt instruction — and it walks PAST `SetSleepTurns`, so it eats both copies even though they are not adjacent — while `identify_sleep_talk_called` regenerates ONE, the divergence sat at INDEX 0. That is why the class always measured 0 `shape_branch_is_prefix_of_tail` and 0 `shape_tail_is_prefix_of_branch`. Guard: `state.use_damage_dealt && !choice.sleep_talk_move` (`third_party/poke-engine-gen3-sleeptalk-damage-dealt-double-reset.patch`).

**REACHABILITY IS NARROW AND WORTH STATING, because it bounds where the world failures can have come from.** `Side::deserialize` hardcodes `damage_dealt: DamageDealt::default()` and `Side::serialize` does not emit the field at all, so any consumer that hands the engine a *serialized* state presents a ZERO carry and cannot reach the guarded block on that ply. That is both `branch_events` and `env_step` (`rust/pokezero-search/src/{events,envstep}.rs`, each opening with `parse_state` → `State::deserialize`), and `env_step` additionally returns `post_state: state.serialize()`, so the carry cannot survive between successive calls either. `State::deserialize` DOES call `set_conditional_mechanics`, so the FLAG is set correctly from movesets while the VALUE is zeroed. The only reachable path is the in-memory tree fold (`model.rs` / `tree.rs`, which apply and reverse instructions on a live `State` without re-serialising), at **depth >= 2** — a ply-1 `set_damage_dealt` is what supplies the carry ply 2 then doubles.

**Measured on the 8,903-branch Sleep-Talk probe population, on the shipping 73-patch stack:** `none_matched` **8,613 → 0**. Those 8,613 refusals partition exactly into **8,292 attributed callee + 321 `ambiguous`** (8,292 + 321 = 8,613); the remaining **290** of the 8,903 branches were never in this class in either build — no Sleep Talk callee walk is reached on them — and are NOT part of that partition, so 8,903 does not itself partition. Recovery is 96.3 % of the refused class, 93.1 % of all branches. (On the pre-rebase 72-patch stack the same probe read 8,651 / 8,361 → 8,040 + 321; #1166's immobilizer markers de-collapse branches that `combine_duplicate_instructions` had merged, which raises every total. The figures above are the ones that ship.) All 321 ambiguous are confirmed genuine multi-candidate collisions — two distinct callees produce a byte-identical tail against the probe's `State::default()` stat block, which has no typing and uniform stats — and were previously MASKED by `none_matched` firing first, because with the duplicate present no candidate matched at all.

**On end-state neutrality, the honest decomposition** (5,184 probe cells, patched vs guard-reverted on the same 73-patch vendor). The obvious metric is blind to the field being repaired, since `Side::serialize` omits `damage_dealt`: on `(percentage, serialized end state)` 5,172 of 5,184 cells are identical. Including `damage_dealt` explicitly, only **594 of 5,184 are identical and 4,590 differ** — that is the repair, not a regression. Aggregating mass per observable end state, the distribution is **preserved in all 5,184 cells** (largest per-end-state deviation 1e-6 pp, f32 noise); only the branch PARTITION moves, in 12 cells. **Counter is the ONLY channel that can exist, by exhaustion over the readers.** gen3 has exactly three readers of `damage_dealt`; everything else is inside `reset_damage_dealt` / `set_damage_dealt`, the writers. **Counter** gates only on `move_category == Physical` — satisfied by the reset — and then returns `damage * 2` ungated on sign: -274 against 0. **Mirror Coat** gates on `move_category == Special`, and the category write is an absolute set, the one sub-field that IS idempotent under doubling, so it returns `Some(0)` under both engines. **Focus Punch** (`choice_effects.rs:266`) needs `damage > 0`, which -137 and 0 both fail; its `!hit_substitute` clause cannot rescue the difference either, because a corrupted `hit_substitute == true` survives only when `set_damage_dealt` did not run — it overwrites both fields absolutely — and that is precisely the case where no damage was dealt. Measurement agrees with the proof: over an exhaustive 4,368-cell sweep (all 364 callee triples from a 14-move pool × {Counter, Mirror Coat, Tackle} × 2 HP × 2 move orders) the projection differs in **70 cells, every one with a Counter defender and none with Mirror Coat**, spanning **26 distinct callee triples** and including **18 full-HP cells** — so this is not one moveset and not a low-HP corner.

**All 26 differing triples contain Shadow Ball**, which identifies the trigger and closes the one discrepancy between this sweep and the diagnosis run: a Ghost move against the probe's typeless `State::default()` mon is a 0x immunity, so its arm is a byte-identical no-op that `combine_duplicate_instructions` can fold into an unrelated arm — the same folding whose removal by #1166's immobilizer markers takes the class total from 8,651 to 8,903. The 72-patch diagnosis reported `[SURF, THUNDER, SWORDSDANCE]` as a differing cell; on the shipping 73-patch stack it is not, while its Shadow-Ball-bearing sibling `[SURF, THUNDER, SHADOWBALL]` is. That earlier example is **superseded by the de-collapse, not wrong**, and it is not a fixture difference — re-running with and without the sleeper's Leftovers changes nothing. In every differing cell the guarded engine is the correct one, computing Counter's fixed damage from `damage_dealt.damage == 0` rather than from `-137`. **No campaign-era rate is claimed here** — these are probe-population branch counts, not a fallback rate. | R | REACHABLE. `sleeptalk` on **40 of 220** species. | **yes as annotation** — `strict:sleeptalk_union_branch` 126 dev / 105 holdout |
