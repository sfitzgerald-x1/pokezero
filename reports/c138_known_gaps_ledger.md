...
**Shape of the document.** §3 carries **81 rows** (58 engine/renderer/leaf, 23 harness/process);
...
RETIRED.** C152 disposes **G8** and both open arms of **G33b** as consciously retired — the
mechanism is real and measured, its observed consequence is zero in every population this program
is permitted to measure, and the row is retained as a named property rather than as an open gap.
An audit has already reversed one "closed" that was really a fourth category, so a retirement is
labelled as one and always carries three things: the measurement, the scope, and the condition
under which it comes back.
