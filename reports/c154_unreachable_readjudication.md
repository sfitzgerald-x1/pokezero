...
`scripts/c154_unreachable_readjudication.py` → `tests/data/c154_unreachable_readjudication.json`,
...
`tests/data/c154_unreachable_readjudication.json`. **This report deliberately does not restate
...
reasoned from a bucket's *name* and never traced its caller: `heal_drain_or_shellbell` has one
production emitter, the Sleep Talk ambiguous-tail path, and **0 of 393 sets** pair `sleeptalk` with
a drain move, so the bucket is *unemittable*, not merely unambiguous.
...
`tests/test_unreachable_readjudication.py`, 25 tests, gated in
`.github/workflows/engine-fidelity-gates.yml` with an exact `Ran 25 tests` guard and a no-skip
...
fifteen citations in a single merge; that is the shape this exists to prevent.
...
3. R22's judgement that the crate's `move_fails_encore` list being gen3's `failencore`-flagged set
   *makes the shipped list correct* is not machine-checkable; the set equality is.
...
**Mutation battery: 12 applied, 12 caught, enumerated in the pin's docstring.** ⚠ **Number 8
survived the first version.** A retracted sentence re-inserted hard-wrapped *and lower-cased* walked
past a guard that already normalised whitespace — C153's own mutation 24, defeating a guard written
by someone who had just read about it. Recorded rather than tidied away.
...
`rust/pokezero-search/src/leaf.rs` gained 31 lines (#1202, `49c31855`) — one of the 11 crate sources
the fingerprint covers — so a rebuild of the committed tree at `main` now stamps
`9517aab98d56a9babe0f261cd83685b9fce20e513c8c5a023436b20052b72cda` at the same 74 patches. No
...
`reports/artifacts/` gains nothing: this pass writes one artifact, and it lives under `tests/data/`
so that `tests/test_never_fired_counter_census.py` does not read its pool counts as counter firings.
That exclusion is asserted by the pin rather than left to the filesystem — a convenience field of
exactly this shape nearly inverted 46 verdicts in C153.
