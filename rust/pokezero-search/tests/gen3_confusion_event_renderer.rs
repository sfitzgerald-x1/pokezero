...
/// The OTHER sub-case must be reachable, and must be labelled `none_matched`.
///
/// Independent review found the `none_matched` arm was dead as far as the suite
/// was concerned: replacing it with `panic!()` left all 336 tests green, and
/// swapping the two literals did too. A sub-case split whose second label is
/// never executed cannot be trusted to be measuring what it says.
///
/// Petal Dance reaches it for a real reason, not an artificial one. It is a
/// locking move, so the engine's branch carries lock bookkeeping that
/// re-generating the candidate in isolation does not reproduce; no candidate's
/// regenerated tail equals the observed tail. That is exactly the defect class
/// `none_matched` exists to name -- the replay diverging from what the engine
/// actually did -- and, unlike `ambiguous`, it is potentially fixable in the
/// renderer without an engine change. Which is the whole reason the split is
/// worth having.
///
/// Note `branch_on_damage: true`, unlike every other fixture in this file. With
/// damage branching off the tail is reproducible and the callee identifies
/// cleanly -- so the divergence only exists on the code path production search
/// actually runs. A fixture built on the file's default helpers would have
/// reported "unreachable" and been wrong.
#[test]
fn the_sleeptalk_none_matched_subcase_is_reachable_and_named() {
    let mut state = confused_state(Choices::SLEEPTALK);
    state
        .side_two
        .volatile_statuses
    ...
    state
        .side_two
        .get_active()
        .replace_move(PokemonMoveIndex::M1, Choices::PETALDANCE);
    ...
    for branch in &branches {
        ...
        for reason in &r.attribution_unsafe {
            ...
            assert_eq!(
                ...
                "an unreproducible tail must report `none_matched`, not \
                 `ambiguous` -- the two need OPPOSITE fixes: {:?}",
                r.attribution_unsafe
            );
            saw_none_matched = true;

            // Same contract check as the ambiguous fixture: the measurement
            // channel splits, the differential's tag does not.
            assert!(
                r.lossy.iter().any(|x| x == "sleeptalk_called_unidentified"),
                "the differential matches the lossy tag EXACTLY; it must stay \
                 unsplit: {:?}",
    ...
    assert!(
        saw_none_matched,
        "fixture no longer reaches `none_matched`; the arm is untested again"
    );
}
