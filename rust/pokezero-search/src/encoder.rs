...
/// records as the larger half: ~60 allocations per leaf remained against ~48 removed. Taking all
/// eight here, since #1249's rewrite of this file forced #1219 to be re-applied anyway and
/// shipping the known-partial half would have been a choice rather than an inheritance.
#[inline]
    ...
    for (slot, candidate) in mons.iter().take(limit).enumerate() {
        ...
        if candidate.active() {
            ...
            if layout.is_v4() {
                if as_bool(get(
                    md,
                    per_role(role, "self_must_recharge", "opponent_must_recharge"),
                )) {
            ...
            if layout.is_v3() {
                for (self_key, opp_key, column, divisor) in [
                    (
                        "self_stall_counter",
                        "opponent_stall_counter",
                        "NUMERIC_STALL_COUNTER",
                        8.0,
                    ),
                    (
                        "self_confusion_elapsed",
                        "opponent_confusion_elapsed",
                        "NUMERIC_CONFUSION_TURNS",
                        5.0,
                    ),
                    (
                        "self_encore_elapsed",
                        "opponent_encore_elapsed",
                        "NUMERIC_ENCORE_TURNS",
                        6.0,
                    ),
                    (
                        "self_wrap_trap_elapsed",
                        "opponent_wrap_trap_elapsed",
                        "NUMERIC_WRAP_TRAP_TURNS",
                        5.0,
                    ),
                ] {
                    let value = as_i64(get(md, per_role(role, self_key, opp_key)));
                    if value != 0 {
                        grid.set_num(
                            token,
                ...
                if as_bool(get(
                    md,
                    per_role(role, "self_meanlook_trap", "opponent_meanlook_trap"),
                )) {
                    grid.set_num(
                        token,
            ...
            if layout.is_v4() {
                for (self_key, opp_key, column) in [
                    (
                        "self_truant_loaf",
                        "opponent_truant_loaf",
                        "NUMERIC_TRUANT_LOAF",
                    ),
                    (
                        "self_choice_locked",
                        "opponent_choice_locked",
                        "NUMERIC_CHOICE_LOCKED",
                    ),
                    (
                        "self_item_swapped",
                        "opponent_item_swapped",
                        "NUMERIC_ITEM_SWAPPED",
                    ),
                ] {
                    if as_bool(get(md, per_role(role, self_key, opp_key))) {
                ...
                for (self_key, opp_key, column) in [
                    ...
                    let value = as_f64(get(md, per_role(role, self_key, opp_key)));
                    if value > 0.0 {
                        grid.set_num(token, layout.num_col(column)?, value.min(1.0));
                    }
                }
                ...
                let last_move = str_or_empty(get(
                    md,
                    per_role(role, "self_last_used_move", "opponent_last_used_move"),
                ));
                ...
                let traced = str_or_empty(get(
                    md,
                    per_role(role, "self_traced_ability", "opponent_traced_ability"),
                ));
                if !traced.is_empty() {
                    grid.set_cat(
                        token,
